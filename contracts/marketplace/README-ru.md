...
Marketplace принимает два связанных с продлением потока TON-комиссий от контрактов сделок:
- `RenewalFeeMessage` для ручных продлений (`0.1 TON` за продление, излишек может быть возвращен владельцу)
- `AutoRenewPrepayMessage` для предоплаты итераций автопродления (`iterations * 0.1 TON`, излишек может быть возвращен владельцу)
